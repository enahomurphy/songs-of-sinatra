var details = [
  {
    name : "Lagos Invasion",
    info : "ndustry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries,"
  },
  {
    name : "Ikeja Stampade",
...
$(".event .main-event ul li").on("click", function() {
    ...
    $(".event .event-info").addClass("animated rotateInDownRight")
    $(".event .event-info h3").text(details[pos].name)
    $(".event .event-info p").text(details[pos].info)

    $this.on("mouseleave", function () {
      $(".event .event-info").removeClass("animated rotateInDownRight")
    })
})
